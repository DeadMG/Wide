        ...
        if (m->OperatorOverloads.find(ty) != m->OperatorOverloads.end()) {
            ...
            for (auto set : overset) {
                for (auto func : set.second) {
                    if (set.first > access)
                        continue;
                    resolvable.insert(analyzer.GetCallableForFunction(analyzer.GetWideFunction(func.get(), this, GetOperatorName(ty), nullptr, [](Wide::Parse::Name, Wide::Lexer::Range, std::function<std::shared_ptr<Expression>(Expression::InstanceKey key)>) { return nullptr; })));
                }
            }
    ...
    AddHandler<const Parse::ModuleOverloadSet<Parse::Function>>(a.MultiObjectHandlers, [](const Parse::ModuleOverloadSet<Parse::Function>* overset, Analyzer& analyzer, Module* lookup, Parse::Access access, std::string name, Lexer::Range where) -> std::shared_ptr<Expression> {
        ...
        for (auto&& map : overset->funcs) {
            ...
            for (auto&& func : map.second)
                resolvable.insert(analyzer.GetCallableForFunction(analyzer.GetWideFunction(func.get(), lookup, name, nullptr, [](Wide::Parse::Name, Wide::Lexer::Range, std::function<std::shared_ptr<Expression>(Expression::InstanceKey key)>) { return nullptr; })));
        }
