...
std::shared_ptr<Expression> ClangTemplateClass::ConstructCall(std::shared_ptr<Expression> val, std::vector<std::shared_ptr<Expression>> args, Context c){
    return CreateResultExpression([=](Function* f) {
        ...
        for (auto&& x : args) {
            if (auto con = dynamic_cast<ConstructorType*>(x->GetType(f))) {
                auto clangty = con->GetConstructedType()->GetClangType(*from);
                if (!clangty) throw InvalidTemplateArgument(x->GetType(f), c.where);
            ...
            if (auto in = dynamic_cast<Integer*>(x.get())) {
                ...
                types.push_back(x->GetType(f));
                continue;
            }
            throw InvalidTemplateArgument(x->GetType(f), c.where);
        }
