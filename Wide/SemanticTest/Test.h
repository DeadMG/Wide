...
template<typename T, typename U> void Test(Wide::Semantic::Analyzer& a, Wide::Semantic::Module* higher, const Wide::AST::Module* root, T errorfunc, U& mockgen) {
    ...
    for(auto overset : root->functions) {
        for(auto fun : overset.second->functions) {
            if (fun->args.size() == 0) {
                Try([&] { a.GetWideFunction(fun, mod)->BuildCall(Wide::Semantic::ConcreteExpression(), std::vector<Wide::Semantic::ConcreteExpression>(), a, fun->where.front()); }, errorfunc);
                continue;
            }
            ...
            try {
                ...
                if (concexpr.size() == fun->args.size()) {
                    ...
                    for(auto x : concexpr)
                        types.push_back(x.t);
                    Try([&] { a.GetWideFunction(fun, mod, std::move(types))->BuildCall(Wide::Semantic::ConcreteExpression(), std::move(concexpr), a, fun->where.front()); }, errorfunc);
                }
            } catch(...) {}
        }
    }
    for(auto decl : root->decls) {
        if (auto use = dynamic_cast<Wide::AST::Using*>(decl.second)) {
            Try([&] { mod->BuildValueConstruction(a, root->where.front()).AccessMember(use->name, a, root->where.front()); }, errorfunc);
        }
        if (auto module = dynamic_cast<Wide::AST::Module*>(decl.second)) {
            Try([&] { mod->BuildValueConstruction(a, root->where.front()).AccessMember(module->name, a, root->where.front()); }, errorfunc);
